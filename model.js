...
module.exports = class Model {
	...
	toJSON(no_attributes, strict) {
		...
		( keys || [] ).forEach( key => {
			if(no_attributes && _.includes(attribute_keys, key)) { return; }
	...
	/**
	 * @param  { string } key [ the key of value retriving from attributes ]
	...
	setValues(values) {
		...
		let set = (key) => {
			...
			value = ( attribute && type !== 'function' && isFunction(value) ) ? value(key, type) : value;

			let default_value = attribute && attribute.default_value;
			default_value = ( attribute && type !== 'function' && isFunction(default_value) ) ? default_value(key, type) : default_value;

			let final_value =  exists(value) ? value : default_value;
			final_value = cloneObject(final_value);
	...
	test(attr, key) {
		...
		if(ModelValidator[attr.type](this[key], attr.options)) { if(!attr.custom) { return; } }
		else { return `${key} should be of type ${attr.type}`; }

		// Custom Validation Tests
		if(attr.custom) { return this.custom_validation(this[key], attr.custom, attr.options); }

		// Default return if any unspecified error
		return `${key} of type ${attr.type} has an undefined error`;
	}
};
