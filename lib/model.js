...
module.exports = class Model {
	...
	deleteAttribute(key) {
		...
		/**
	 * @return { object } [ returns the options definiing the model ]
	 */
	getOptions() {
	 ...
	 * @param { value } value [ the model validation object to add to options ]
	 * @return { model } [ this instance of the model ]
	 */
	addOption(key, value) {
	...
	setValues(values) {
		...
		set.forEach((key) => {
			...
			if(
				(attribute && attribute.input && isOutput) ||
				(attribute && attribute.output && isInput)
			) { return; }
		...
		set.forEach((key) => {
			let attribute = this.getAttribute(key);
			if(!attribute || !attribute.alias) { return; }

			let reduceAlias = (a, b) => [ a,  this.getValue(b) ].join(' ').trim();

			this[key] = !isArray(attribute.alias) ?
						this.getValue(attribute.alias) :
						attribute.alias.reduce(reduceAlias, '');
		});
	...
	isValid(return_errors) {
		...
		Object.keys(attributes).forEach((key) => {
			...
			if(
				(attr && attr.input && isOutput) ||
				(attr && attr.output && isInput)
			) { return; }
	...
	test(attr, key) {
		...
		if(!attr.required && !exists(value)) { return; }
		if( attr.required && !exists(value)) { return `${key} of type ${attr.type} is required`; }

		// Validate type and value is of type
		if(!ModelValidator[attr.type](value, attr.options)) { return `${key} should be of type ${attr.type}`; }
		...
		if(
			...
			!attr.choices.includes(value)
		) { return `${key} of type ${attr.type} should be one of ${attr.choices}`; }

		// Validate each element in an array / object
		if(attr.items && (isArray(value) || isObject(value))) {
			let testItemValues = (key) => this.test.call(value, attr.items, key);
